...
- **Wiki-links — closed grammar**, single line, inside `[[ … ]]`:
  ...
  escapes `\#`, `\|`, `\]` are valid in all three parts. Targets are whitespace-trimmed and NFC-normalized before resolution (D8). A target that violates the D8 path grammar still produces an edge — dangling, with `reason = 'invalid'`.
- **Table-cell caveat (GFM interaction):** the table extension splits cells on unescaped `|` *before* inline parsing, so `[[a|b]]` unescaped inside a cell yields two cells and no link; write `[[a\|b]]`. Documented, deterministic, golden-cased.
- **Transclusions:** `![[…]]`, same grammar.
- **Markdown links** `[text](target)`: a target with a URL scheme (`https:`, `mailto:`, …) is external — no vault edge. A scheme-less target is a candidate vault path: strip an optional `.md` suffix, then resolve per D8, producing an edge of kind `mdlink` (the plan's §6 edge kind). Autolinked bare URLs don't exist under D1.
...
3. for `list_item`: **exclude descendant blocks' spans** — the item's content is its own direct content only. Descendants carry their own hashes; editing a leaf sub-item must not ripple "modified" through every ancestor item (that would blind RFC-004's exact-match stage and break Law-5 embedding reuse);
4. strip the block-ID marker (` ^id`) if present — adding an ID must not change content identity;
...
- Access functions (transient, over the type): blocks with kind/ord/heading_path/content/content_hash/span/attrs; extracted links, tags, properties, preamble.
...
4. **`vault-syntax-extraction`** — golden corpus for D3/D4: wiki-links (anchors, aliases, escapes, whitespace-trimming, NFD targets), the reference-definition collision case, table-cell `|` broken/escaped pairs, transclusions, tags (`**bold**#tag` non-match, line-initial `#tag` match), block-ID markers (per-kind, including the excluded kinds), `mdlink` internal/external split, frontmatter (valid, invalid-as-content, reserved keys). This suite seeds Phase 2's `extraction-correctness` corpus (plan §16).
