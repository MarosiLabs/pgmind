...
**Batching is an implementation freedom with one boundary: it MUST NOT narrow what pgmind accepts.** *Added 2026-08-05 (post-batching review).* Steps 6-8 may issue one statement per lane instead of one per row — and should, since per-row statements cost ~3 SPI executions per link and one per block. But the set of documents the write path stores is fixed by `pgmind.max_document_bytes` (RFC-002 D9), not by the marshalling: sending rows through a single `jsonb` argument silently imposed PostgreSQL's `JENTRY_OFFLENMASK` ceiling (256 MB − 1, on one string *and* on an array's element total) where the per-row `text` binding had the 1 GB datum limit, so a note the previous implementation stored was rejected with a bare `54000` that did not even name the lane. Two rules follow, both pinned by `lane_batching_survives_chunk_boundaries`: unbounded columns (`tile.raw`, `block.content`) travel as `text[]`, which has no per-element ceiling; and every lane flushes in bounded chunks, which also caps how much of a note one statement holds in backend memory. Chunking makes the pre-order of `doc.blocks` load-bearing — a non-deferrable FK is checked at end of *statement*, so a minted child may not land in an earlier chunk than its minted parent.
...
- **Separator synthesis.** After any structural splice at top level (move, insert, split/merge of top-level blocks), every tile **at the splice seam** except the note's final tile MUST end with a blank line, synthesized if absent; the note's final tile keeps exactly the trailing trivia it had before the operation. The seam is the inserted/relocated tile run plus the tile immediately preceding it — the only boundaries the splice created.
  ...
  *Amended 2026-08-05 (post-batching review).* "Keeps exactly the trailing trivia it had" binds the **fragment-taking ops too**, not just separator synthesis: `update_block`, `split_block` and `merge_blocks` terminated their replacement with a newline unconditionally, so any of them applied to the last block of a note that did not end in one appended a byte the note never contained (`alpha\n\nbeta` → `alpha\n\nBETA\n`). `insert_blocks` and `move_block` already preserved it, so the three ops also disagreed with the other two. Normative: the replacement carries the terminator the replaced region carried — which everywhere but the end of the source is a newline, because without it the replacement would merge into what follows. `verify_note` cannot see this class of defect (it compares the stored lanes against a fresh parse of the stored bytes, and both agree after the extra byte is written); only the caller's original bytes disagree, so it is pinned by `final_block_ops_keep_trailing_trivia` in the identity-semantics gate instead.
...
- **Variables:** per-row heap widths for the six tables (measured, not estimated: `pgtoast`-aware, via `pg_column_size`/`pg_total_relation_size` on the synthetic vault), index sizes, blocks-per-note and edges-per-block distributions from the generator: **23 blocks, 4 links and 4 tags per note** (`eval/harness.py:synthetic_note`, verified against `knowledge.stats()` on the measured corpus). *Amended 2026-08-05 (post-batching review).* This bullet claimed "Walkthrough A's shape: ~23 blocks, ~3 links, ~0.2 tags per note" while the generator emitted 4 and 4 — a 20× overstatement of tag density in the same section that publishes what extraction costs. The generator's shape is the one recorded here; it is link- and tag-denser than Walkthrough A, which makes the extraction share an upper bound rather than a typical case, and the ablation below reports it separately for exactly that reason.
...
- **Write cost is measured by the harness, not by prose.** *Amended 2026-08-05 (post-batching review).* What stood here published `179.5 ± 1.5 → 202.8 ± 1.2 notes/s`, "median of 5 runs", and a three-way ablation — none of which the committed harness could produce. It timed **one** pass and published a single sample, so the RFC was quoting a procedure that existed only in the session that ran it, and the artifact it names as the deliverable disagreed with the prose (204.1 vs 202.8 ± 1.2). A number this section cannot regenerate is not a published number. The measurement now lives in `eval/harness.py` (`write_cost_measurements`), its full output is in `capacity-model-v1.json` under `write_cost`, and everything below is quoted from it — regenerate with `make eval`.

  **Procedure:** five rounds × three note shapes, each pass writing a 500-note corpus into an **empty database** (every write a create; a rewrite takes the other `write_note` branch, so mixing the two measures neither), release build, single connection, staging loaded untimed; then eight successive passes into one database for the growth curve.

  | shape | ms/note (median of 5) | share |
  |---|---|---|
  | full note — 23 blocks, 4 links, 4 tags | 3.902 | |
  | same note, links and tags removed | 2.714 | |
  | one-block note | 1.072 | |
  | ⇒ extraction (4 links + 4 tags, incl. the D5 creation-repair pass) | 1.188 | 30% |
  | ⇒ 22 further blocks + 7 further tiles | 1.642 | 42% |
  | ⇒ per-note fixed cost | 1.072 | 27% |

  Run-to-run spread of the full shape across the five passes is **0.173 ms** (±2%), so all three attribution terms are resolvable; the artifact records the spread next to them precisely so a future term smaller than it cannot be quoted as if it were real. (The first version of this bench ran every pass into one *growing* database and reported the median. The passes were monotonically slowing — 3.9 → 9.5 ms/note — so the "median" was the cost at the median vault size, and the ablation charged that growth to whichever shape ran last, reporting extraction at 0.05 ms/note. The split into empty-vault passes plus a separate growth curve is what makes the ablation an ablation.)

- **Throughput is published with the vault size it was measured at.** 500-note create pass into an **empty** vault: **256.3 notes/s** (3.902 ms/note, median of 5, range 3.856–4.029). The single 10k-note pass that builds the capacity corpus: **200.2 notes/s** (4.995 ms/note) — ~28% more per note, reproduced across every run of this bench, and the reason a "notes/s" figure without a vault size is not a fact about pgmind.

  What that 28% is made of is **not** yet established. Eight successive 500-note passes into one vault (`growth_ms_per_note_by_vault_size`) climbed monotonically 3.905 → 5.460 ms/note on one run and then, on the next, rose to 5.024 by 2 500 notes and fell back to 3.870 by 3 500 — so on the reference machine the per-step curve is inside the drift, and only the aggregate difference survives repetition. Characterising it properly (index maintenance on `block`/`edge`/`tag` vs resolution probes vs page-cache effects, on quiet hardware) is **Phase 3's storage-growth benchmark**, which RFC-005 owns. Recorded here rather than smoothed away: this section publishes the measurement, including the part of it that is still noise.

- **What batching bought, and why statement count is no longer the lever.** Collapsing the four row lanes from one statement per row to one per lane removed ~39 SPI statements per corpus note (55 → 16, counted with `pg_stat_statements`) and moved a 10k-note pass from 176.1 to 200.1 notes/s (3 runs each, measured on both builds when the change landed) — **≈ 17 µs per statement removed**, marginal. That is the number to reason with. The "~95 µs per statement" this section used to publish was a whole-note *average* — parse, hashing, WAL, index maintenance and the RI checks Postgres issues on its own, all divided by a statement count — and using it as a marginal cost overstated what statement count controls by ~5×. At 17 µs, deleting *every* remaining statement would save ~0.28 ms of a 3.90 ms note.

  Statement inventory, counted from the source and corroborated by `pg_stat_statements`: a **create** issues 13 statements for a one-block link-free note and 16 for a corpus-shaped one; a **rewrite** issues 12; five are reads either way. The head swap and the D5 creation repair are on **mutually exclusive branches** — a create never swaps a head, a rewrite never repairs — so no single write pays both, and any inventory that bills them together (as this section's did) describes a call that cannot happen.

- **The 2 k notes/s design target is not reachable per-call, by batching or otherwise.** A one-block, link-free note — nothing left to batch — costs 1.072 ms, and 2 k notes/s means 0.5 ms. The remaining cost is per-*call* work, not per-statement: parse, hashing, the revision row, WAL, commit. The target describes the **Phase 4 bulk CLI import path**, which amortizes that fixed cost across many notes per call; comparing per-call `knowledge.write()` against it is a category error, which is exactly why this number is reported and not gated.
