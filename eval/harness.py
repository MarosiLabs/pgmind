...
def suite_gate_selftest():
    ...
    # 4. The rebinding gate asserts three things; each must be breakable. These
    #    run on fabricated cases rather than the real corpus -- a negative
    #    control that needed the corpus to be wrong would be untestable.
    body = """
**why:** fabricated.
...
"""
    fake = Path("selftest.md")
    good = rebinding.Case(fake, "st", body % "")
    obs = {"before": [(0, "paragraph", "B0", "alpha", None)],
           "after": [(0, "paragraph", "A0", "beta", None),
                     (1, "paragraph", "B0", "gamma", None)],
           "carried": [(0, 1)]}
    scored = rebinding.score(good, obs)
    cases["rebinding_scorer_counts_a_misbinding_as_wrong"] = (
        scored["fp"] == 1 and scored["tp"] == 0 and scored["misbound"] == [(0, 1)])
    try:
        rebinding.Case(fake, "st", body % "- before[0] => after[1]\n")
        cases["rebinding_parser_rejects_a_double_binding"] = False
    except rebinding.CorpusError:
        cases["rebinding_parser_rejects_a_double_binding"] = True
...
def suite_rebinding_corpus():
    """RFC-004 §5 Part B: the adversarial edit corpus. The published match-rate
    IS the deliverable -- the gate is that the number exists, is honest, and is
    ...
    cases = {case.id: case for case in rebinding.load_cases()}
    rows, observations = rebinding.run_corpus(c, "pgmind_rebind")

    stale = [r["case"] for r in rows
             if not rebinding.index_is_current(cases[r["case"]], r["index"])]
    ...
    out = {"status": "ok" if ok else "fail",
           "cases": len(rows),
           "recall": everything["recall"],
           "precision": everything["precision"],
           # The headline for Part B: bindings no deterministic rule can make.
           "recall_inferred": everything["recall_inferred"]["recall"],
